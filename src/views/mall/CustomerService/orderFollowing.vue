  ...
  <div id="wrapper">
    <div class="search-wrapper">
      <el-form :inline="true" :model="orderSearch" size="small" label-width="80px">
        ...
        <el-form-item label="商户账号:" class="commonItem">
          <el-input v-model="orderSearch.sellerUserName" placeholder="请输入商户账号" clearable></el-input>
        </el-form-item>
    ...
    <div id="orderWrap" v-for="(item, index) in tableListData" :key="index">
      ...
      <div class="orderBody">
        ...
        <div class="shu"></div>
        <div class="status" :alt=item.orderStatus>{{item.orderStatus}}</div>
        <div class="shu"></div>
        <div class="remarks">{{item.operationRemark}}</div>
        <div class="operation">
          <el-button type="primary" size="middle" @click="seeDetail(index)">查看</el-button>
          <el-button type="primary" size="middle" @click="remove(index)">移除</el-button>
          <el-button type="primary" size="middle" @click="Remarks(index)">备注</el-button>
        </div>
      </div>
    </div>
    <div id="orderFooter">
    ...
    <!-- 移除跟踪订单 -->
    <el-dialog title="移除跟踪订单" :visible.sync="removeOrderTrack" :center="true" :modal-append-to-body="false" :append-to-body="true" width="500px">
      <el-form :model="orderTrackData" ref="removeData" :rules="removeOrderRules" size="small" label-width="50px">
        <p class="trackOrderID">订单编号：<span >{{orderTrackData.orderId}}</span></p>
        <p class="content trackcontent">请备注移除理由:</p>
        ...
        <el-form-item class="form-button-wrapper">
          <el-button type="default" @click="cleanRemove" icon="el-icon-close">取 消</el-button>
          <el-button type="primary" @click="saveRemove" icon="el-icon-check">确 定</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    <!-- 操作备注 -->
    <el-dialog title="操作备注" :visible.sync="orderRemark" :center="true" :modal-append-to-body="false" :append-to-body="true" width="500px">
      <el-form :model="orderRemarkData" ref="remarkData" :rules="rulesRemark" size="small" label-width="50px">
        <p class="trackOrderID">订单编号：<span >{{orderRemarkData.orderId}}</span></p>
        <p class="content trackcontent">请输入操作说明:</p>
        <el-form-item label="" prop="productDes">
          <el-input type="textarea" :rows="5" v-model.trim="orderRemarkData.content" style="width:90%"></el-input>
        </el-form-item>
        <el-form-item class="form-button-wrapper">
          <el-button type="default" @click="cleanRemark" icon="el-icon-close">取 消</el-button>
          <el-button type="primary" @click="saveRemark" icon="el-icon-check">确 定</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
export default {
  data() {
    return {
      ...
      removeOrderRules: {
        content: [
          { required: true, message: '请输入移除说明', trigger: 'blur' },
        ],
      },
      orderRemark: false, // 订单备注
      orderRemarkData: {
        content: ''
      },
      rulesRemark: {
  ...
  methods: {
    ...
    remove(index) {
      ...
      this.removeOrderTrack = true;
    },
    cleanRemove() {
      this.removeOrderTrack = false;
    },
    // 确认移除
    saveRemove() {
      let _self = this;
      ...
      _self.$refs['removeData'].validate((valid) => {
        if (valid) {
    ...
    Remarks(index) {
      this.itemIndex = index;
      this.orderRemarkData.content = this.tableListData[index].operationRemark;
      this.orderRemarkData.orderId = this.tableListData[index].orderId;
    ...
    saveRemark() {
      ...
      let param = {
        orderId: _self.tableListData[index].orderId,
        remark: _self.orderRemarkData.content,
        orderStatus: _self.tableListData[index].orderStatus,
      };
      _self.$refs['remarkData'].validate((valid) => {
        if (valid) {
          java.post(`/mall_manage/back/logService/insertLogRemark`, param).then((res) => {
            if (res.status === 200 && res.data.resultCode === '0') {
              _self.$message({
                message: '备注成功',
                type: 'success'
              });
              _self.orderRemarkData.content = _self.orderRemarkData.content;
              _self.orderRemark = false;
              this.getTableData();
            }
          });
        }
    ...
    exportOrder() {
      ...
      if (_self.exportList.length > 0) {
        _self.$confirm('此操作将导出勾选中的订单, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          let orderId = _self.exportList.join(',');
          window.open(`${process.env.MALL_BASE_HOST}/mall_manage/back/logService/exportExcleLogOrder?orderId=` + orderId);
          _self.exportList = [];
          _self.allCheckList = document.getElementById('all');
          _self.checked = document.getElementsByName('check');
          for (let i = 0; i < _self.tableListData.length; i++) {
            if (_self.allCheckList.checked) {
    ...
    getTableData() {
      ...
      java.post(`/mall_manage/back/logService/selectLogOrder`, param).then((res) => {
        if (res.status === 200 && res.data.resultCode === '0') {
          _self.tableListData = res.data.resultData.data;
          _self.total = res.data.resultData.total;
          // eslint-disable-next-line array-callback-return
          // eslint-disable-next-line complexity
          let orderInfo = _self.tableListData.map(function(item, index, data) {
            // 订单状态
