...
export default {
  ...
  methods: {
    ...
    setSelectedList(data, proudctNum, sellerUserId) {
      ...
      else if (data !== false && (data[0].getByself || data[0].getCommon || data[0].getFast || data[0].transnational) && sellerUserId === '深圳市康美人生医药连锁有限公司') {
      ...
      else if (data !== false && sellerUserId !== '深圳市康美人生医药连锁有限公司') {
        _self.$message({
          message: '请选择药店来源为：深圳市康美人生医药连锁有限公司',
          type: 'warning'
        });
      }
      else if (data === false) {
