  ...
  <div id="specialProduct">
    ...
    <el-dialog :title=title :visible.sync="specialShow" :modal-append-to-body="false" :append-to-body="true" width="500px" center @close="cancelSpecial">
      <el-form ref="addData" :model="addForm" :rules="addRules" :inline="true" label-width="100px">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="专题图片" :prop="'specialImg'" :rules="{ required: true, message: '请选择图片', trigger: 'change'}">
              <uploadImg :imgUrl="addForm.specialImg" :fileName="'specialImg'" @uploadSuccess="uploadspecialImg" @removeImg="removespecialImg"></uploadImg>
              <el-input class="left" type="hidden" v-model="addForm.specialImg"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
export default {
  ...
  computed: {
    columns() {
      ...
      let columns = [
        ...
        { field: 'specialImg', title: '专题图片', width: 100, titleAlign: 'center', columnAlign: 'center', isResize: true },
  ...
  methods: {
    // 上传图
    uploadspecialImg(data, imgUrl) {
      this.addForm.specialImg = imgUrl;
    },
    // 删除图
    removespecialImg(url) {
      this.addForm.specialImg = url;
    },
    // 清空查询
    clean() {
    ...
    customCompFunc(params) {
      ...
      if (params.type === 'edit') { // 修改
        ...
        _self.$router.push('/drug-control/gift_productManage?id=' + params.rowData.specialId);
      }
    },
    // 取消专题
    cancelSpecial() {
