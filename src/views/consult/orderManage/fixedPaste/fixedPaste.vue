    ...
    <div class="search-wrapper">
      <el-form :inline="true"  size="small" label-width="100px" :model="searchInfo">
        <el-form-item label="膏方名称">
          <el-input  placeholder="膏方名称" v-model.trim="searchInfo.name"  @keyup.enter.native="searchProduct"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getColumns() {
      let columns = [
        { field: 'id', title: '编号', width: 180, titleAlign: 'center', columnAlign: 'center', isResize: true },
    ...
    customCompFunc(params) {
      ...
      if (params.type === 'use') {
        if (params.rowData.status === 0) {
          this.$confirm('确定将状态改为禁用?', '提示', {
            ...
            ylajax.get('/api/ordermanage/ChangePasteFixedTemplate', { params: useData }).then((res) => {
              if (res.status === 200 && res.data.code === 0) {
                this.$message({
                  message: '状态成功修改为禁用',
                  type: 'success'
      ...
      if (params.type === 'delete') {
        ...
        this.$confirm('确定删除该条记录?', '提示', {
          ...
          ylajax.get('/api/ordermanage/DelPasteFixedTemplateById', { params: delData }).then((res) => {
            if (res.status === 200 && res.data.code === 0) {
              this.$message({
                message: '删除成功',
                type: 'success'
