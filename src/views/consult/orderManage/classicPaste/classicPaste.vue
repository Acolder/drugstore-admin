...
export default {
  ...
  methods: {
    ...
    getFormData() {
      ...
      ylajax.get('/api/ordermanage/GetPasteStandardCategoryList', { params: fromData }).then((res) => {
    ...
    getColumns() {
      let columns = [
        { field: 'id', title: '编号', width: 30, titleAlign: 'center', columnAlign: 'center', isResize: true },
        { field: 'name', title: '膏方名称', width: 150, titleAlign: 'center', columnAlign: 'center', isResize: true },
        { field: 'description', title: '膏方介绍', width: 250, titleAlign: 'center', columnAlign: 'center', isResize: true },
        { field: 'remark', title: '备注', width: 150, titleAlign: 'center', columnAlign: 'center', isResize: true },
        { field: 'state', title: '状态', width: 80, titleAlign: 'center', columnAlign: 'center', componentName: 'table-classic-prescription', isResize: true },
        { field: 'operation', title: '操作', width: 200, titleAlign: 'center', columnAlign: 'center', componentName: 'table-classic-salesmanList', isResize: true }
      ];
    ...
    customCompFunc(params) {
      ...
      if (params.type === 'use') {
        if (params.rowData.state === 0) {
          this.$confirm('确定将状态改为禁用?', '提示', {
            ...
            ylajax.get('/api/ordermanage/ChangePasteStandardCategory', { params: useData }).then((res) => {
              if (res.status === 200 && res.data.code === 0) {
                this.$message({
                  message: '状态成功修改为禁用',
                  type: 'success'
        ...
        } else if (params.rowData.state === 1) {
      ...
      if (params.type === 'delete') {
        ...
        this.$confirm('确定删除该条记录?', '提示', {
          ...
          ylajax.get('/api/ordermanage/DelPasteStandardCategoryById', { params: delData }).then((res) => {
            if (res.status === 200 && res.data.code === 0) {
              this.$message({
                message: '删除成功',
                type: 'success'
