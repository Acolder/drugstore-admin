<template>
  <div>
    ...
    <!-- 查看详情 -->
    <el-dialog title="资讯详情" :visible.sync="newsDetail" width="60%" :center="true" :modal-append-to-body="false" class="details">
      <el-row>
...
export default {
  ...
  methods: {
    ...
    getDetail(id) {
      let _self = this;
      ajax.get(`/api/admin/getNewsDetail/` + id).then(res => {
    ...
    customCompFunc(params) {
      ...
      } else if (params.type === 'release') { // 发布
    ...
    release(Ids) {
      let _self = this;
      ajax.post(`/api/admin/AuditNews`, Ids).then(res => {
        if (res.status === 200 && res.data.code === 0) {
          _self.$message({
    ...
    remove(Ids) {
      let _self = this;
      ajax.post('/api/admin/deleteNews', Ids).then(res => {
        if (res.status === 200 && res.data.code === 0) {
          _self.$message({
            message: res.data.message,
            type: 'success'
