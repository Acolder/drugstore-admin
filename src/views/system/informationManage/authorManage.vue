<template>
  <div id="cardList">
    ...
    <div class="toolbar-wrapper">
      <el-button-group>
        <el-button type="default" size="small" icon="el-icon-plus" @click="addDoctor">添加作者</el-button>
    ...
    <el-dialog :title="title" :visible.sync="doctorShow" :modal-append-to-body="false" :append-to-body="true" @close="cancel" width="450px" center>
      <el-form :model="formData" size="small" ref="formData" :rules="addrules" label-width="100px">
        <el-form-item label="用户手机号:" prop="mobile" class="source" >
          <el-input v-model="formData.mobile" placeholder="请用户手机号" clearable></el-input>
        </el-form-item>
        <el-form-item label="作者简介:" prop="author_introduction"  class="source" style="margin:20px 0;">
  ...
  data() {
    ...
    let authorLength = (rule, value, callback) => {
      if (value) {
        ...
        return callback(new Error('请输入作者简介'));
      }
    };
    // 手机号验证
    ...
    return {
      ...
      addrules: {
        ...
        author_introduction: [{ required: true, validator: authorLength, trigger: 'blur' }],
      },
      title: '',  // 新增、编辑标题
      doctorShow: false,
      doctorDetail: '',
  ...
  computed: {
    columns() {
      ...
      let columns = [
        ...
        { field: 'mobile', title: '账号', width: 100, titleAlign: 'center', columnAlign: 'center', isResize: true, overflowTitle: true },
        { field: 'author_introduction', title: '作者简介', width: 50, titleAlign: 'center', columnAlign: 'center', isResize: true },
