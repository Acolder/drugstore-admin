<template>
  <div class="category">
    ...
    <!-- 新增修改分类 -->
    <el-dialog :title="categoryTitle" :visible.sync="categoryShow" :modal-append-to-body="false" width="400px" center @close="closeCategory">
      <el-form ref="categoryfrom" :model="categoryData" size="small" label-width="100px" :rules="rules">
        <el-form-item label="分类名称：" prop="categoryname" style="width:85%;margin-bottom: 18px;">
          <el-input v-model="categoryData.categoryname" placeholder="请输入分类名称" clearable></el-input>
        </el-form-item>
        <el-form-item label="分类排序：" prop="sort" style="width:85%;margin-bottom: 18px;">
          <el-input v-model="categoryData.sort" placeholder="请输入分类排序" clearable></el-input>
        </el-form-item>
        <el-form-item label="是否显示：" prop="isshow" style="width:85%;margin-bottom: 18px;">
        ...
        <el-form-item class="footer">
          <el-button type="default" @click="categoryShow = false" icon="el-icon-circle-close">取消</el-button>
          <el-button type="primary" @click="addQuery" icon="el-icon-circle-check">确定</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    addQuery() {
      let _self = this;
      let url = '/api/news/saveNewsCategory';
      _self.$refs['categoryfrom'].validate((valid) => {
        if (valid) {
          ajax.post(url, _self.categoryData).then(res => {
